...
/--
Fill the right-hand side of a production with states: terminals are kept, the `k`-th
entry of the right-hand side becomes `fill k` when it is a nonterminal.  This is the
`δ`-generator `δ_F` of Algorithm 3.2.
-/
def fillRhs (rhs : List SigmaElt) (fill : Nat → String) : List (Beta String) :=
  rhs.zipIdx.map fun xi =>
    match xi.1 with
    | .term a => .term a
    | .nt _   => .state (fill xi.2)

/-- `δ_F(target, fill, f)`: the transition for symbol `f` with the given target state. -/
def deltaGen (g : CFG) (target : String) (fill : Nat → String) (f : Sym) :
    Option (Transition String) :=
  (g.prodOfSym f).map fun p => ⟨target, f, fillRhs p.2 fill⟩
...
def genTA (g : CFG) (oa : Oa) (op : OrderMap) (excludeTrivial : Bool := true) : TA String :=
  ...
  let trivStates := (triv.filterMap fun f => (g.prodOfSym f).map Prod.fst).dedup
  ...
  let nonTriv : List (Transition String) := op.flatMap fun oss =>
    oss.2.filterMap fun s =>
      ...
      | some p => deltaGen g (stateName oss.1)
                    (fun k => if k = p then stateName (oss.1 + 1) else stateName oss.1) s
      | none   => deltaGen g (stateName oss.1) (fun _ => stateName oss.1) s
  ...
  let cycTrans : List (Transition String) := (g.highToLow op).filterMap fun pr =>
    deltaGen g (stateName pr.2.2) (fun _ => stateName pr.1.2) pr.2.1
